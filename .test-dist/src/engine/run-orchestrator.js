...
import { PENDING_INITIAL_DISPATCH_STATE_FIELD, PENDING_INITIAL_DISPATCH_VERSION, PENDING_INITIAL_DISPATCH_VERSION_STATE_FIELD, STATE_SCHEMA_VERSION, } from "../constants.js";
import { InvalidConfigError, ProtocolError, RunLockedError, StateMigrationBlockedError, StateMissingError, } from "../errors/concrete.js";
import { nodeSqliteDriver } from "../persistence/sqlite/node-sqlite-driver.js";
import { acquireOwnership, releaseOwnership, } from "../persistence/sqlite/ownership.js";
import { bootstrapNewRunAtomic, migrateLegacyRunAtomic, } from "../persistence/sqlite/run-bootstrap.js";
import { openRunDatabase } from "../persistence/sqlite/run-database.js";
import { commitState, projectAuthoritativeStateFenced, readAuthoritativeState, } from "../persistence/sqlite/run-state-store.js";
import { clock } from "../services/clock.js";
import { createLogger } from "../services/logger.js";
import { cleanupOldRuns, resolveRunDir } from "../services/run-dir.js";
import { generateRunId } from "../services/run-id.js";
import { migrateV3ToV4, readStateSnapshot, } from "../services/state-io.js";
import { summarizeZodError, validateResult } from "../services/validator.js";
import { doExit, isTestExitSignal } from "./context.js";
...
import { parseArgv, validateConfig, validateExternalRunId, } from "./preflight.js";
...
const productionRunOrchestratorDependencies = {};
...
function seedLegacyStateToSqlite(runDir, runId, state) {
    ...
    try {
        ...
        if (migrateResult.kind === "ALREADY_ESTABLISHED") {
            ...
            runDb.close();
            throw new StateMissingError("Legacy migration: run already established by another process", { runId, orchestratorName: state.orchestratorName });
        }
        ...
        if (migrateResult.kind === "INCOMPLETE_EXISTING_BOOTSTRAP") {
            runDb.close();
            throw new StateMissingError(`Legacy migration: incomplete existing bootstrap — ${migrateResult.details}`, { runId, orchestratorName: state.orchestratorName });
        }
...
function stateRecordToStateFile(record, runDir) {
    const base = {
    ...
    if (record.schemaVersion === 3) {
        const migrated = migrateStateFileV3ToV4(result, runDir);
        return migrated;
    }
    return result;
}
...
function migrateStateFileV3ToV4(parsed, runDir) {
    try {
...
async function runInitialMode(config, argv, dependencies) {
    ...
    if (config.stateSchema) {
        ...
        if (!validation.ok) {
            throw new InvalidConfigError(`config.initialState fails stateSchema: ${summarizeZodError(validation.error)}`, {
    ...
    if (bootstrapResult.kind !== "BOOTSTRAPPED") {
        ...
        if (bootstrapResult.kind === "ACTIVE_CONFLICT") {
            emitRunLockedError(new RunLockedError(`Run is locked by another process, lease until ${new Date(bootstrapResult.leaseUntilEpochMs).toISOString()}`, {
                ownerPid: 0,
        ...
        throw new ProtocolError(`Failed to bootstrap run: ${bootstrapResult.kind}`, { runId, orchestratorName: config.name });
    }
    ...
    const initialFile = {
        schemaVersion: committedState.schemaVersion ??
            STATE_SCHEMA_VERSION,
        runId: committedState.runId ?? runId,
        orchestratorName: committedState.orchestratorName ?? config.name,
        startedAt: committedState.startedAt ?? nowIso,
        startedAtEpochMs: committedState.startedAtEpochMs ?? nowEpoch,
        lastTransitionAt: committedState.lastTransitionAt ?? nowIso,
        lastTransitionAtEpochMs: committedState.lastTransitionAtEpochMs ?? nowEpoch,
        currentPhase: committedState.currentPhase ?? config.initial,
        phasesExecuted: committedState.phasesExecuted ?? 0,
        accumulatedDurationMs: committedState.accumulatedDurationMs ?? 0,
        data: committedState.data ?? config.initialState,
        usedLabels: committedState.usedLabels ?? [],
    };
    ...
    try {
        ...
        projectAuthoritativeStateFenced(runDb.connection, handle, runDir, committed.stateRevision, committed.stateDigest, undefined, dependencies.projectionDependencies);
        ...
        try {
            cleanupOldRuns(cwd, config.name, config.retentionDays ?? 7, runId, config.runDirRoot);
        }
        catch {
            // best-effort
        }
        ...
        runDb.close();
        if (releaseResult.kind !== "SUCCESS" &&
            releaseResult.kind !== "STALE_HANDLE") {
            throw new ProtocolError("Initial setup or dispatch claim failed and ownership release also failed", {
                runId,
                orchestratorName: config.name,
...
async function runResumeMode(config, argv, dependencies) {
    if (!argv.runId) {
    ...
    if (!dbExists) {
        ...
        let snapshot;
        try {
            snapshot = readStateSnapshot(runDir, config.stateSchema);
        }
        catch (err) {
            if (err instanceof StateMigrationBlockedError) {
                throw new StateMigrationBlockedError(`v3→v4 migration incomplete — cannot create authoritative SQLite DB: ${err.message}`, {
                    reason: err.reason,
                    runId,
                    orchestratorName: config.name,
                });
        ...
        if (snapshot.state.runId !== runId) {
            throw new ProtocolError(`RUN_DIR mismatch — state.runId=${snapshot.state.runId}, argv.runId=${runId}`, { runId, orchestratorName: config.name });
        }
        if (snapshot.state.orchestratorName !== config.name) {
            throw new ProtocolError(`orchestrator name mismatch — state.orchestratorName=${snapshot.state.orchestratorName}, config.name=${config.name}`, { runId, orchestratorName: config.name });
        }
        ...
        const preRead = readAuthoritativeState(runDb.connection);
        if (preRead.state !== null) {
            ...
            if (preRead.state.runId !== runId) {
                runDb.close();
                throw new ProtocolError(`DB identity mismatch — incarnation runId=${preRead.state.runId}, argv.runId=${runId}`, { runId, orchestratorName: config.name });
            }
            if (preRead.state.orchestratorName !== config.name) {
                runDb.close();
                throw new ProtocolError(`DB identity mismatch — incarnation orchestratorName=${preRead.state.orchestratorName}, config.name=${config.name}`, { runId, orchestratorName: config.name });
            }
            ...
            if (acquireResult.kind === "ACTIVE_CONFLICT") {
                runDb.close();
                emitRunLockedError(new RunLockedError(`Run is locked by PID ${acquireResult.ownerPid}, lease until ${new Date(acquireResult.leaseUntilEpochMs).toISOString()}`, {
            ...
            if (acquireResult.kind !== "ACQUIRED") {
                runDb.close();
                throw new ProtocolError(`Failed to acquire ownership: ${acquireResult.kind}`, { runId, orchestratorName: config.name });
            }
            ...
            if (!fs.existsSync(legacyStatePath)) {
                throw new StateMissingError("SQLite DB exists but has no state row, and state.json is also missing", { runId, orchestratorName: config.name });
            }
            let snapshot;
            try {
                snapshot = readStateSnapshot(runDir, config.stateSchema);
                ...
                if (err instanceof StateMigrationBlockedError) {
                    throw new StateMigrationBlockedError(`v3→v4 migration incomplete — cannot recover incomplete SQLite bootstrap: ${err.message}`, {
                        reason: err.reason,
                        runId,
                        orchestratorName: config.name,
                    });
            ...
            if (snapshot.state === null) {
                throw new StateMissingError("state.json missing — cannot recover incomplete SQLite bootstrap", { runId, orchestratorName: config.name });
            }
    ...
    try {
        const readResult = readAuthoritativeState(runDb.connection);
        ...
        if (readResult.state.schemaVersion === 3) {
            const migrationResult = migrateV3ToV4(readResult.state, runDir);
            if (migrationResult.kind === "MIGRATED") {
                ...
                const migratedRecord = {
                    ...
                    ...(maybeMigrated.terminalResult !== undefined
                        ? {
                            terminalResult: maybeMigrated.terminalResult,
                        }
                        : {}),
                };
                ...
                if (commitResult.kind !== "COMMITTED") {
                    throw new ProtocolError(`v3→v4 migration commit failed: ${commitResult.kind}`, {
                        runId,
                        orchestratorName: config.name,
                        cause: commitResult.kind === "DB_FAILURE"
                            ? commitResult.cause
                            : undefined,
                    });
                }
                authoritativeRecord = commitResult.committed.state;
        ...
        projectAuthoritativeStateFenced(runDb.connection, handle, runDir, authoritativeRecord.stateRevision, authoritativeDigest ?? "", undefined, dependencies.projectionDependencies);
        if (state.runId !== runId) {
            throw new ProtocolError(`RUN_DIR mismatch with argv — state.runId=${state.runId}, argv.runId=${runId}`, { runId, orchestratorName: config.name });
        }
        if (state.orchestratorName !== config.name) {
            throw new ProtocolError(`orchestrator name mismatch — state.orchestratorName=${state.orchestratorName}, config.name=${config.name}`, { runId, orchestratorName: config.name });
        }
        fs.mkdirSync(path.join(runDir, "external-requests"), { recursive: true });
        fs.mkdirSync(path.join(runDir, "external-results"), { recursive: true });
        fs.mkdirSync(path.join(runDir, "accepted-external-resolutions"), {
        ...
        if (releaseResult.kind !== "SUCCESS" &&
            ...
            process.stderr.write(`[turnlock] ownership release failed: ${releaseResult.kind}` +
                (releaseResult.kind === "DB_FAILURE"
...
export async function runOrchestratorInternal(config, argv, dependencies) {
...
export async function runOrchestrator(config) {
    try {
        ...
        await runOrchestratorInternal(config, argv, productionRunOrchestratorDependencies);
    }
    catch (err) {
        if (isTestExitSignal(err))
            return;
        try {
            handleTopLevelError(err, config);
        }
        catch (e) {
            if (isTestExitSignal(e))
                return;
        }
    }
}
