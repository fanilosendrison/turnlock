...
export async function handleExternalRequest(ctx, state, result, accumulatedDurationMs) {
    ...
    try {
        ...
        const prepared = prepareJsonArtifact(ctx.runDir, "external-request-manifest", manifest);
        // 2. Install immutable blob.
        installPreparedArtifact(ctx.runDir, prepared);
        // 3. Build the protocol block in memory (not emitted yet).
        const canonicalManifestPath = path.join(ctx.runDir, "external-requests", `${request.label}.json`);
        const publication = (() => {
            try {
                return {
                    ok: true,
        ...
        if (isTestExitSignal(error))
            throw error;
        await emitFatalError(ctx, durableState ?? state, state.currentPhase, error);
        return undefined;
    }
}
//# sourceMappingURL=external-request-handler.js.map