...
function resolveRunDirRoot(cwd, configRoot) {
    const envRoot = process.env[RUN_DIR_ROOT_ENV_VAR];
    const root = envRoot !== undefined && envRoot !== ""
        ? envRoot
...
export function resolveRunDir(cwd, orchestratorName, runId, runDirRoot) {
    if (cwd === "")
        throw new InvalidConfigError("cwd cannot be empty");
    return path.join(resolveRunDirRoot(cwd, runDirRoot), orchestratorName, runId);
}
export function cleanupOldRuns(cwd, orchestratorName, retentionDays, currentRunId, runDirRoot) {
