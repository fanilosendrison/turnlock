...
// ---------------------------------------------------------------------------
function atomicInstallImmutable(targetPath, bytes) {
    // Fast path: target already exists → verify integrity
    try {
        const existing = fs.readFileSync(targetPath);
        if (existing.length !== bytes.length) {
            throw new ArtifactIntegrityError(`artifact collision: existing blob at ${targetPath} has different size (${existing.length} vs ${bytes.length})`);
        }
        const existingDigest = sha256Digest(existing);
        const newDigest = sha256Digest(bytes);
        if (existingDigest !== newDigest) {
            throw new ArtifactIntegrityError(`artifact collision: existing blob at ${targetPath} has different digest (${existingDigest} vs ${newDigest})`);
        }
    ...
    try {
        ...
        if (err.code === "EEXIST") {
            ...
            const existing = fs.readFileSync(targetPath);
            if (existing.length !== bytes.length) {
                throw new ArtifactIntegrityError(`artifact collision on link: existing blob has different size`);
            }
            const existingDigest = sha256Digest(existing);
            const newDigest = sha256Digest(bytes);
            if (existingDigest !== newDigest) {
                throw new ArtifactIntegrityError(`artifact collision on link: existing blob has different digest`);
            }
            return "existing-match";
        }
        throw err;
...
export function prepareJsonArtifact(_runDir, kind, value) {
 ...
 */
export function installPreparedArtifact(runDir, artifact) {
 ...
 */
export function readAndVerifyArtifact(runDir, ref) {
    ...
    try {
        ...
        throw new ArtifactIntegrityError(`artifact blob unreadable at ${ref.relativePath}: ${err instanceof Error ? err.message : String(err)}`, { cause: err });
    }
    if (bytes.length !== ref.sizeBytes) {
        throw new ArtifactIntegrityError(`artifact size mismatch: expected ${ref.sizeBytes} bytes, got ${bytes.length} at ${ref.relativePath}`);
    }
    const actualDigest = sha256Digest(bytes);
    if (actualDigest !== ref.digest) {
        throw new ArtifactIntegrityError(`artifact digest mismatch: expected ${ref.digest}, got ${actualDigest} at ${ref.relativePath}`);
    }
    return bytes;
}
...
export function artifactAbsolutePath(runDir, digest) {
...
export function validateArtifactRef(ref, expectedKind) {
    // Structural checks first.
    validateArtifactStructure(ref);
    // Kind must match the field-specific expectation.
    if (ref.kind !== expectedKind) {
        throw new ArtifactIntegrityError(`artifact kind mismatch: expected ${expectedKind}, got ${ref.kind}`);
    }
}
...
function validateArtifactStructure(ref) {
    ...
    if (ref.digestAlgorithm !== "sha256") {
        throw new ArtifactIntegrityError(`unsupported digest algorithm: ${ref.digestAlgorithm}`);
    }
    // Digest format: sha256:<64 hex chars>
    const digestPattern = /^sha256:[0-9a-f]{64}$/;
    if (!digestPattern.test(ref.digest)) {
        throw new ArtifactIntegrityError(`artifact digest has invalid format: ${ref.digest.slice(0, 20)}...`);
    }
    // Digested path vs actual path
    const expectedPath = artifactRelativePath(ref.digest);
    if (ref.relativePath !== expectedPath) {
        throw new ArtifactIntegrityError(`artifact relativePath does not match digest: expected ${expectedPath}, got ${ref.relativePath}`);
    }
    // Path confinement — must be relative, no traversal, under artifacts/sha256.
    if (path.isAbsolute(ref.relativePath)) {
        throw new ArtifactIntegrityError(`artifact relativePath must be relative: ${ref.relativePath}`);
    }
    const segments = ref.relativePath.split(path.sep);
    if (segments.includes("..")) {
        throw new ArtifactIntegrityError(`artifact relativePath must not contain "..": ${ref.relativePath}`);
    }
    if (segments[0] !== "artifacts" || segments[1] !== "sha256") {
        throw new ArtifactIntegrityError(`artifact relativePath must start with artifacts/sha256: ${ref.relativePath}`);
    }
    // Media type
    if (ref.mediaType !== "application/json") {
        throw new ArtifactIntegrityError(`unsupported media type: ${ref.mediaType}`);
    }
    // Size
    if (typeof ref.sizeBytes !== "number" ||
        ...
        throw new ArtifactIntegrityError(`artifact sizeBytes invalid: ${ref.sizeBytes}`);
    }
}
