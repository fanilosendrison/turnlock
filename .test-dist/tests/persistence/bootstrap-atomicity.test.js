...
import { bootstrapNewRunAtomicCore, InjectedBootstrapFailure, migrateLegacyRunAtomicCore, } from "../../src/persistence/sqlite/run-bootstrap-core.js";
...
function assertFullyEstablished(runDb, handle, committed) {
    // incarnation
...
describe("bootstrap atomicity", () => {
    test("A — bootstrap réussi: all three tables populated, handle valid", () => {
        ...
        try {
            ...
            assert.strictEqual(authRead.state.runIncarnationId, result.handle.incarnationId);
            // Clean release.
            releaseOwnership({
                db: ctx.runDb.connection,
...
function makeDeterministicIdGenerator() {
    let counter = 0;
    return () => {
        counter++;
        ...
        test(`rollback at ${point} — zero rows, no handle, re-bootstrap succeeds`, () => {
            ...
            try {
                ...
                if (result.kind === "DB_FAILURE") {
                    ...
                    assert.strictEqual(result.cause.point, point);
                }
...
describe("legacy migration atomicity", () => {
    test("K — migration réussie: all three tables populated, handle valid", () => {
        ...
        const ownRow = ctx.runDb.connection
            .prepare("SELECT acquired_at_epoch_ms, lease_until_epoch_ms FROM run_ownership WHERE singleton = 1")
            .get();
        ...
        assert.strictEqual(authRead.state.lastTransitionAt, "2020-01-01T00:01:00.000Z");
...
describe("old implementation partial DB handling", () => {
    test("N — incarnation+ownership present, state absent, no state.json → INCOMPLETE_BOOTSTRAP", () => {
        ...
        const ownRow = ctx.runDb.connection
            ...
            .get();
        assert.strictEqual(ownRow.ownership_status, "HELD");
        const fence = typeof ownRow.fence_token === "bigint"
            ? ownRow.fence_token
            : BigInt(ownRow.fence_token);
    ...
    test("R — committed_by_owner_token matches handle.ownerToken", () => {
        ...
        try {
            ...
            const stateRow = ctx.runDb.connection
                .prepare("SELECT committed_by_owner_token, committed_by_fence_token FROM run_state WHERE singleton = 1")
                .get();
            assert.strictEqual(stateRow.committed_by_owner_token, result.handle.ownerToken);
            const fence = typeof stateRow.committed_by_fence_token === "bigint"
                ? stateRow.committed_by_fence_token
                : BigInt(stateRow.committed_by_fence_token);
...
describe("incarnation identity", () => {
    ...
    test("T — three-table coherence after migration", () => {
        ...
        // run_incarnation
        const incRow = ctx.runDb.connection
            .prepare("SELECT run_id, incarnation_id FROM run_incarnation WHERE singleton = 1")
            .get();
        assert.strictEqual(incRow.run_id, RUN_ID);
        assert.strictEqual(incRow.incarnation_id, handle.incarnationId);
        // run_ownership
    ...
    test("U — committed state incarnation matches handle", () => {
        ...
        assert.strictEqual(result.committed.incarnationId, result.handle.incarnationId);
        // committed.state.runIncarnationId must also match.
        assert.strictEqual(result.committed.state.runIncarnationId, result.handle.incarnationId);
        releaseOwnership({ db: ctx.runDb.connection, handle: result.handle });
        ctx.cleanup();
    });
    // -----------------------------------------------------------------------
    ...
    test("V — two independent migrations produce distinct incarnations", () => {
        ...
        try {
            ...
            assert.notStrictEqual(first.handle.incarnationId, second.handle.incarnationId);
            releaseOwnership({
                db: ctx1.runDb.connection,
                handle: first.handle,
            });
...
//# sourceMappingURL=bootstrap-atomicity.test.js.map