...
import { buildEntrypointSource, countProtocolBlocks, createE2EWorkspace, parseSingleProtocolBlock, readJsonFile, } from "../helpers/e2e-process.js";
...
function seedV3State(runDb, incarnationId, v3StateJson) {
    unsafeEnsureInitialStateRow(runDb.connection, incarnationId, 3, // v3
    v3StateJson, NOW_EPOCH, NOW_ISO);
}
/** Build a StateRecord from a migrated v4 object + the original record. */
function buildMigratedRecord(original, migrated) {
    const record = {
        ...original,
...
describe("v3→v4 SQLite migration (resume path)", () => {
    test("successful migration: v3 delegation state with real manifest file", () => {
        ...
        try {
            ...
            writeFileSync(join(ctx.dir, "delegations", "rev-0.json"), manifestContent);
            // Acquire ownership.
            ...
            // Migrate v3→v4.
            const migrationResult = migrateV3ToV4(rState, ctx.dir);
            ...
            assert.strictEqual(existsSync(join(ctx.dir, artifact.relativePath)), true);
            // Verify: revision incremented.
            assert.strictEqual(aState.stateRevision, String(BigInt(rState.stateRevision) + 1n));
            // Clean release.
    ...
    test("no-op migration: v3 clean state (no pending records) migrates to v4", () => {
        ...
        try {
            ...
            if (migrationResult.kind !== "MIGRATED")
                return;
            assert.strictEqual(migrationResult.state.schemaVersion, STATE_SCHEMA_VERSION);
            // Commit the migrated v4 state.
            const migratedRecord = buildMigratedRecord(rState, migrationResult.state);
            const commitResult = commitState({
                db: ctx.runDb.connection,
    ...
    test("blocked migration: v3 delegation with missing manifest file", () => {
        ...
        try {
            ...
            assert.strictEqual(releaseResult.kind === "SUCCESS" ||
                releaseResult.kind === "STALE_HANDLE", true);
        }
        finally {
            ctx.cleanup();
        }
...
describe("v3→v4 migration via runOrchestrator --resume (E2E)", () => {
    ...
    function v3DelegationState(runId, orchestratorName) {
        return {
            schemaVersion: 3,
            runId,
            orchestratorName,
    ...
    test("full chain via dbExists: v3 in SQLite → migration commit (rev 1) → terminal (rev 2)", async () => {
        ...
        try {
            ...
            const entrypoint = workspace.writeEntrypoint("v3v4-e2e.ts", buildEntrypointSource(`
...
`));
            ...
            writeFileSync(join(runDir, "delegations", "review-0.json"), manifestContent);
            // 4. Write the result file so runHandleResume can consume it.
            writeFileSync(join(runDir, "results", "review-0.json"), JSON.stringify({ approved: true }));
            ...
            unsafeEnsureInitialStateRow(seedDb.connection, acquireResult.handle.incarnationId, 3, // v3 schema version
            JSON.stringify(v3State), NOW_EPOCH, NOW_ISO);
            ...
            const result = await workspace.runEntrypoint(entrypoint, ["--resume", "--run-id", runId], { timeoutMs: 15000 });
            ...
            const state = readJsonFile(join(runDir, "state.json"));
            ...
            const manifestDigest = createHash("sha256")
                ...
                .digest("hex");
            const expectedBlobPath = join(runDir, "artifacts", "sha256", manifestDigest.slice(0, 2), `${manifestDigest.slice(2)}.json`);
            ...
            const output = readJsonFile(join(runDir, "output.json"));
    ...
    test("blocked migration via dbExists: v3 in SQLite + missing manifest → StateMigrationBlockedError", async () => {
        ...
        try {
            ...
            const entrypoint = workspace.writeEntrypoint("v3v4-e2e-blocked.ts", buildEntrypointSource(`
interface State { approved: boolean; reviewed: boolean }
...
`));
            ...
            const v3State = v3DelegationState(runId, orchestratorName);
            v3State.pendingDelegation.manifestPath =
                "delegations/nonexistent.json";
            unsafeEnsureInitialStateRow(seedDb.connection, acquireResult.handle.incarnationId, 3, JSON.stringify(v3State), NOW_EPOCH, NOW_ISO);
            releaseOwnership({
                db: seedDb.connection,
                handle: acquireResult.handle,
            });
    ...
    test("legacy .lock present + no DB → resume blocked, SQLite DB not created", async () => {
        ...
        try {
            // 1. Write entrypoint.
            const entrypoint = workspace.writeEntrypoint("lock-guard-blocked.ts", buildEntrypointSource(`
interface State { approved: boolean; reviewed: boolean }
...
`));
            ...
            const manifestDigest = createHash("sha256")
                .update(manifestContent)
                .digest("hex");
            const blobDir = join(runDir, "artifacts", "sha256", manifestDigest.slice(0, 2));
            mkdirSync(blobDir, { recursive: true });
            writeFileSync(join(blobDir, `${manifestDigest.slice(2)}.json`), manifestContent);
            const stateV4 = {
                schemaVersion: STATE_SCHEMA_VERSION,
                runId,
                orchestratorName,
            ...
            assert.strictEqual(block.action, "ERROR");
            assert.strictEqual(block.runId, runId);
            assert.strictEqual(block.fields.errorKind, "legacy_lock_migration_blocked");
    ...
    test("legacy .lock absent + no DB → normal migration proceeds, SQLite ownership acquired", async () => {
        ...
        try {
            // 1. Write entrypoint.
            const entrypoint = workspace.writeEntrypoint("lock-guard-ok.ts", buildEntrypointSource(`
interface State { approved: boolean; reviewed: boolean }
...
`));
    ...
    test("DB present + .lock present → MixedOwnershipProtocolError, fail-closed", async () => {
        ...
        try {
            const entrypoint = workspace.writeEntrypoint("mixed-own-e2e.ts", buildEntrypointSource(`
interface State { approved: boolean; reviewed: boolean }
...
`));
            ...
            unsafeEnsureInitialStateRow(seedDb.connection, acquireResult.handle.incarnationId, STATE_SCHEMA_VERSION, JSON.stringify(v4State), NOW_EPOCH, NOW_ISO);
            ...
            assert.strictEqual(block.action, "ERROR");
            assert.strictEqual(block.runId, runId);
            assert.strictEqual(block.fields.errorKind, "mixed_ownership_protocol_detected");
            ...
            try {
                const afterFence = checkDb.connection
                    .prepare("SELECT fence_token, ownership_status FROM run_ownership WHERE singleton = 1")
                    .get();
                assert.notStrictEqual(afterFence, undefined);
                assert.strictEqual(typeof afterFence.fence_token === "bigint"
                    ? afterFence.fence_token
                    : BigInt(afterFence.fence_token), typeof beforeFence.fence_token === "bigint"
                    ? beforeFence.fence_token
                    : BigInt(beforeFence.fence_token));
    ...
    test("initial mode on reused RUN_DIR with .lock → blocked before SQLite creation", async () => {
        ...
        try {
            const entrypoint = workspace.writeEntrypoint("initial-lock-e2e.ts", buildEntrypointSource(`
...
`));
            ...
            const result = await workspace.runEntrypoint(entrypoint, ["--run-id", runId], { timeoutMs: 15000 });
            assert.strictEqual(result.exitCode, 1);
            assert.strictEqual(countProtocolBlocks(result.stdout), 1);
            const block = parseSingleProtocolBlock(result.stdout);
            assert.strictEqual(block.action, "ERROR");
...
//# sourceMappingURL=v3v4-migration.test.js.map