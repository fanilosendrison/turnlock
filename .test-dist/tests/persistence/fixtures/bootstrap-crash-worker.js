...
import { bootstrapNewRunAtomicCore, migrateLegacyRunAtomicCore, } from "../../../src/persistence/sqlite/run-bootstrap-core.js";
...
// Helpers
// ---------------------------------------------------------------------------
function makeInitialState(runId, orchestratorName) {
    return {
...
function makeWorkerIdGenerator(runId) {
    let idIndex = 0;
    return () => {
        idIndex++;
...
function main() {
    ...
    const crashPoint = args["crash-point"];
    ...
    if (!runDir ||
        ...
        !orchestratorName ||
        !signalFile) {
        process.stderr.write("Missing required arguments. Need: --run-dir --mode --crash-point --run-id --orchestrator-name --signal-file\n");
        process.exit(1);
    }
    const validatedSignalFile = signalFile;
    if (mode !== "BOOTSTRAP" && mode !== "MIGRATION") {
        process.stderr.write(`Invalid mode: ${mode}. Must be BOOTSTRAP or MIGRATION\n`);
        process.exit(1);
    }
    ...
    try {
        if (mode === "BOOTSTRAP") {
            ...
            process.stdout.write(`${JSON.stringify({ type: "RESULT_RETURNED", kind: result.kind })}\n`);
        }
        else {
            // MIGRATION mode.
            const legacyStateRaw = readFileSync(legacyStateFile, "utf-8");
            const legacyState = JSON.parse(legacyStateRaw);
            // Extract legacy timestamps from the state file.
            const legacyStartedAtEpochMs = legacyState.startedAtEpochMs ?? NOW_EPOCH;
            const legacyStartedAt = legacyState.startedAt ?? NOW_ISO;
            const legacyLastTransitionAtEpochMs = legacyState.lastTransitionAtEpochMs ?? NOW_EPOCH;
            const legacyLastTransitionAt = legacyState.lastTransitionAt ?? NOW_ISO;
...
main();
//# sourceMappingURL=bootstrap-crash-worker.js.map