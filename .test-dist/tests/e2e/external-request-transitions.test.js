...
import { buildEntrypointSource, countProtocolBlocks, createE2EWorkspace, parseSingleProtocolBlock, readEvents, readStateFile, writeExternalResolution, writePromptResult, } from "../helpers/e2e-process.js";
const RUN_IDS = {
...
describe("pending yield cleanup across result kinds", () => {
    test("a delegation emitted after external resolution replaces the external pending record", async () => {
        ...
        const entrypoint = workspace.writeEntrypoint("external-to-delegate.ts", buildEntrypointSource(`
...
`));
        try {
            ...
            expectProtocol(initial.stdout, "REQUEST_EXTERNAL", RUN_IDS.externalToDelegate);
            const runDir = workspace.runDir("e2e-external-to-delegate", RUN_IDS.externalToDelegate);
    ...
    test("an external request emitted after delegation resolution replaces the delegation pending record", async () => {
        const workspace = createE2EWorkspace();
        const entrypoint = workspace.writeEntrypoint("delegate-to-external.ts", buildEntrypointSource(`
interface State { stage: string }
...
`));
        try {
            ...
            const runDir = workspace.runDir("e2e-delegate-to-external", RUN_IDS.delegationToExternal);
            ...
            assert.strictEqual(resumed.exitCode, 0);
            expectProtocol(resumed.stdout, "REQUEST_EXTERNAL", RUN_IDS.delegationToExternal);
            const state = readStateFile(runDir);
    ...
    test("fail after consuming an external resolution clears both pending record kinds", async () => {
        const workspace = createE2EWorkspace();
        const entrypoint = workspace.writeEntrypoint("external-to-fail.ts", buildEntrypointSource(`
interface State { stage: string }
...
`));
        try {
            ...
            const runDir = workspace.runDir("e2e-external-to-fail", RUN_IDS.externalToFail);
            ...
            const error = expectProtocol(resumed.stdout, "ERROR", RUN_IDS.externalToFail);
            assert.strictEqual(error.fields.message, "external result rejected by phase");
            const state = readStateFile(runDir);
            assert.ok(!("pendingExternalRequest" in Object(state)));
            assert.ok(!("pendingDelegation" in Object(state)));
            assert.strictEqual(readEvents(runDir).some((event) => event.eventType === "retry_scheduled"), false);
        }
        finally {
            workspace.cleanup();
        }
...
//# sourceMappingURL=external-request-transitions.test.js.map