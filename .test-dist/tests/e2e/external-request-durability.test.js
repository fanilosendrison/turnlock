...
import { buildEntrypointSource, countProtocolBlocks, createE2EWorkspace, parseSingleProtocolBlock, readEvents, readJsonFile, readStateFile, writeExternalResolution, writeMalformedPromptResult, writePromptResult, } from "../helpers/e2e-process.js";
const RUN_IDS = {
...
describe("external request crash durability", () => {
    test("a crash after consumption leaves the same resolution consumable on the next resume", async () => {
        ...
        const entrypoint = workspace.writeEntrypoint("external-crash-after-consume.ts", buildEntrypointSource(`
...
`));
        try {
            ...
            const runDir = workspace.runDir("e2e-external-crash", RUN_IDS.crashAfterConsume);
            writeExternalResolution(runDir, "external-work", { value: "durable" });
            const crashed = await workspace.runEntrypoint(entrypoint, ["--resume", "--run-id", RUN_IDS.crashAfterConsume], { env: { CRASH_AFTER_CONSUME: "1" } });
            assert.strictEqual(crashed.exitCode, 77);
            assert.strictEqual(crashed.stdout, "");
            const stateAfterCrash = readStateFile(runDir);
            const acceptedResolutionPath = join(runDir, "accepted-external-resolutions", "external-work.json");
            ...
            const acceptedResolutionDigest = stateAfterCrash.pendingExternalRequest?.acceptedResolutionDigest;
            ...
            assert.strictEqual(typeof stateAfterCrash.pendingExternalRequest?.acceptedAt, "string");
            assert.strictEqual(readFileSync(acceptedResolutionPath, "utf-8"), '{"value":"durable"}');
            ...
            try {
                db.exec("UPDATE run_ownership SET lease_until_epoch_ms = 0 WHERE singleton = 1");
            }
            finally {
                db.close();
            }
            ...
            const done = expectProtocol(resumed.stdout, "DONE", RUN_IDS.crashAfterConsume);
            assert.deepStrictEqual(readJsonFile(done.fields.output), { value: "durable" });
            ...
            assert.strictEqual(readEvents(runDir).filter((event) => event.eventType === "external_resolution_validated").length, 2);
        }
        finally {
            workspace.cleanup();
        }
    ...
    test("a resume command failure cannot roll back a committed external request", async () => {
        const workspace = createE2EWorkspace();
        const entrypoint = workspace.writeEntrypoint("external-resume-command-failure.ts", buildEntrypointSource(`
interface State { stage: string }
...
`));
        try {
            const failed = await workspace.runEntrypoint(entrypoint, ["--run-id", RUN_IDS.resumeCommandFailure], { env: { RESUME_COMMAND_BOOM: "1" } });
            assert.strictEqual(failed.exitCode, 1);
            expectProtocol(failed.stdout, "ERROR", RUN_IDS.resumeCommandFailure);
            const runDir = workspace.runDir("e2e-external-resume-command-failure", RUN_IDS.resumeCommandFailure);
            const committed = readStateFile(runDir);
            ...
            if (artifactRef?.relativePath) {
                assert.strictEqual(existsSync(join(runDir, artifactRef.relativePath)), true);
            }
            const resumed = await workspace.runEntrypoint(entrypoint, [
                "--resume",
                "--run-id",
            ...
            assert.strictEqual(resumed.exitCode, 0);
            expectProtocol(resumed.stdout, "REQUEST_EXTERNAL", RUN_IDS.resumeCommandFailure);
...
describe("state v2 migration during resume", () => {
    test("a v2 pending delegation is persisted as v3 under the acquired lock and remains resumable", async () => {
        const workspace = createE2EWorkspace();
        const entrypoint = workspace.writeEntrypoint("state-v2-delegation.ts", buildEntrypointSource(`
interface State { stage: string }
...
`));
        try {
            ...
            const runDir = workspace.runDir("e2e-state-v2-delegation", RUN_IDS.migrationDelegation);
            const statePath = join(runDir, "state.json");
            const current = readJsonFile(statePath);
            const pendingBefore = current.pendingDelegation;
            writeFileSync(statePath, JSON.stringify({ ...current, schemaVersion: 2 }));
            ...
            assert.strictEqual(resumed.exitCode, 0);
            const done = expectProtocol(resumed.stdout, "DONE", RUN_IDS.migrationDelegation);
    ...
    test("a migrated v2 pending delegation keeps its existing retry behavior", async () => {
        const workspace = createE2EWorkspace();
        const entrypoint = workspace.writeEntrypoint("state-v2-delegation-retry.ts", buildEntrypointSource(`
interface State { stage: string }
...
`));
        try {
            ...
            assert.strictEqual(initial.exitCode, 0);
            const runDir = workspace.runDir("e2e-state-v2-retry", RUN_IDS.migrationRetry);
            const statePath = join(runDir, "state.json");
            const current = readJsonFile(statePath);
            writeFileSync(statePath, JSON.stringify({ ...current, schemaVersion: 2 }));
            ...
            const block = expectProtocol(retried.stdout, "DELEGATE", RUN_IDS.migrationRetry);
            const manifest = readJsonFile(block.fields.manifest);
            ...
            assert.strictEqual(readEvents(runDir).some((event) => event.eventType === "retry_scheduled"), true);
        }
        finally {
            workspace.cleanup();
        }
...
//# sourceMappingURL=external-request-durability.test.js.map