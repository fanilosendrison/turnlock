...
function makeRequest(jobCount, label = "batch") {
    return {
        kind: "batch",
        worker: "reviewer",
...
describe("BatchBinding.buildManifest (T-BT-01..05)", () => {
    test("T-BT-01 | 1 job manifest", () => {
        ...
        assert.strictEqual(m.jobs?.[0]?.resultPath, `${RUN_DIR}/results/batch-0/j1.json`);
    });
    test("T-BT-02 | 3 jobs", () => {
    ...
    test("T-BT-04 | 0 jobs → InvalidConfigError", () => {
        ...
        assert.throws(() => batchBinding.buildManifest(req, makeContext()), InvalidConfigError);
    });
    test("T-BT-05 | attempt=2 per-attempt dir", () => {
        const m = batchBinding.buildManifest(makeRequest(3), makeContext({ attempt: 2 }));
        const firstJob = expectJobs(m)[0];
        if (firstJob === undefined)
            assert.fail("expected the first batch job");
...
describe("BatchBinding.buildProtocolBlock (T-BT-06..08)", () => {
    test("T-BT-06 | bloc DELEGATE batch", () => {
        const m = batchBinding.buildManifest(makeRequest(3), makeContext());
        const b = batchBinding.buildProtocolBlock(m, "/tmp/delegations/batch-0.json", "cmd");
...
describe("BatchBinding properties (P-BT-a..c)", () => {
    test("P-BT-a | pure", () => {
        ...
        assert.deepStrictEqual(batchBinding.buildManifest(req, ctx), batchBinding.buildManifest(req, ctx));
    });
    test("P-BT-b | each job resultPath shape", () => {
        const m = batchBinding.buildManifest(makeRequest(3), makeContext({ attempt: 1 }));
...
//# sourceMappingURL=batch-binding.test.js.map