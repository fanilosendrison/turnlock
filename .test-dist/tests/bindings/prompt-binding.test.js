...
describe("PromptBinding.buildManifest (T-PRM-01..03)", () => {
    ...
    test("T-PRM-02 | long prompt preserved", () => {
        ...
        const m = promptBinding.buildManifest({ ...baseRequest, prompt: long }, makeContext());
        assert.strictEqual(m.prompt, long);
    });
    test("T-PRM-03 | attempt=1 per-attempt path", () => {
        const m = promptBinding.buildManifest(baseRequest, makeContext({ attempt: 1 }));
...
describe("PromptBinding.buildProtocolBlock (T-PRM-04..05)", () => {
    test("T-PRM-04 | bloc DELEGATE prompt", () => {
        const m = promptBinding.buildManifest(baseRequest, makeContext());
        const b = promptBinding.buildProtocolBlock(m, "/tmp/delegations/review-foo-0.json", "cmd");
...
describe("PromptBinding properties (P-PRM-a/b)", () => {
    test("P-PRM-a | pure", () => {
        ...
        assert.deepStrictEqual(promptBinding.buildManifest(baseRequest, ctx), promptBinding.buildManifest(baseRequest, ctx));
    });
    test("P-PRM-b | resultPath shape", () => {
        for (const attempt of [0, 1, 4]) {
            const m = promptBinding.buildManifest(baseRequest, makeContext({ attempt }));
            assert.strictEqual(m.resultPath, `${RUN_DIR}/results/review-foo-${attempt}.json`);
        }
    });
});
//# sourceMappingURL=prompt-binding.test.js.map