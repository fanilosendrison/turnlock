...
import { AbortedError, DelegationMissingResultError, DelegationSchemaError, DelegationTimeoutError, ExternalResolutionMalformedError, ExternalResolutionMissingError, ExternalResolutionSchemaError, IndeterminatePhaseExecutionError, InitialDispatchAlreadyClaimedError, InvalidConfigError, LegacyLockMigrationBlockedError, MixedOwnershipProtocolError, PhaseError, ProtocolError, RunLockedError, StateCorruptedError, StateMissingError, StateVersionMismatchError, } from "../../src/errors/concrete.js";
...
describe("error-classifier acceptance (T-EC-01..14)", () => {
    ...
    test("T-EC-04 | StateVersionMismatchError → permanent", () => {
        assert.strictEqual(classify(new StateVersionMismatchError("x")), "permanent");
    });
    ...
    test("T-EC-07 | DelegationMissingResultError → permanent", () => {
        assert.strictEqual(classify(new DelegationMissingResultError("x")), "permanent");
    });
    ...
    test("T-EC-08 | PhaseError(cause=Error) → permanent", () => {
        assert.strictEqual(classify(new PhaseError("x", { cause: new Error("y") })), "permanent");
    });
    test("T-EC-09 | PhaseError(cause=AbortedError) → abort", () => {
        assert.strictEqual(classify(new PhaseError("x", { cause: new AbortedError("y") })), "abort");
    });
    ...
    test("ownership migration guard errors are permanent", () => {
        assert.strictEqual(classify(new LegacyLockMigrationBlockedError("x")), "permanent");
        assert.strictEqual(classify(new MixedOwnershipProtocolError("x")), "permanent");
    });
    test("initial dispatch recovery errors are permanent", () => {
        assert.strictEqual(classify(new IndeterminatePhaseExecutionError("x")), "permanent");
        assert.strictEqual(classify(new InitialDispatchAlreadyClaimedError("x")), "permanent");
    });
...
describe("error-classifier properties (P-EC-a/b)", () => {
    test("P-EC-a | pure over 50 iterations", () => {
        ...
        for (let i = 0; i < 50; i++)
            assert.strictEqual(classify(err), first);
    });
