...
describe("resolveRunDir (T-RD-01..03, T-RD-09..12)", () => {
    test("T-RD-01 | composes canonical path with default root", () => {
        assert.strictEqual(resolveRunDir("/repo", "senior-review", "01HX"), join("/repo", DEFAULT_ROOT, "senior-review", "01HX"));
    });
    test("T-RD-02 | cwd with spaces", () => {
        assert.strictEqual(resolveRunDir("/my repo", "foo", "01H"), join("/my repo", DEFAULT_ROOT, "foo", "01H"));
    });
    ...
    test("T-RD-09 | relative runDirRoot is joined to cwd", () => {
        assert.strictEqual(resolveRunDir("/repo", "orch", "id", ".claude/run/cc-orch"), "/repo/.claude/run/cc-orch/orch/id");
    });
    test("T-RD-10 | absolute runDirRoot ignores cwd prefix", () => {
        assert.strictEqual(resolveRunDir("/repo", "orch", "id", "/abs/path"), "/abs/path/orch/id");
    });
    test("T-RD-11 | env var overrides config argument", () => {
        process.env.TURNLOCK_RUN_DIR_ROOT = ".envroot";
        assert.strictEqual(resolveRunDir("/repo", "orch", "id", ".configroot"), join("/repo", ".envroot", "orch", "id"));
    });
    test("T-RD-12 | empty env var falls back to config/default", () => {
        process.env.TURNLOCK_RUN_DIR_ROOT = "";
        assert.strictEqual(resolveRunDir("/repo", "orch", "id"), join("/repo", DEFAULT_ROOT, "orch", "id"));
        assert.strictEqual(resolveRunDir("/repo", "orch", "id", ".custom"), join("/repo", ".custom", "orch", "id"));
    });
    test("T-RD-14 | empty config runDirRoot falls back to default", () => {
        assert.strictEqual(resolveRunDir("/repo", "orch", "id", ""), join("/repo", DEFAULT_ROOT, "orch", "id"));
    });
});
function touch(path, daysAgo) {
