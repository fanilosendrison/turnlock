...
import { StateCorruptedError, StateMigrationBlockedError, StateVersionMismatchError, } from "../../src/errors/concrete.js";
import { readState, readStateSnapshot, writeStateAtomic, } from "../../src/services/state-io.js";
...
describe("readState (T-SI-01..07)", () => {
    ...
    test("T-SI-02 | valid v2 → migrated typed StateFile", () => {
        const dir = makeTempDir();
        try {
            writeFileSync(join(dir, "state.json"), loadFixture("states/initial-empty.json"));
            const state = readState(dir);
            assert.notStrictEqual(state, null);
            if (state === null)
                return;
            assert.ok(state.schemaVersion >= 3);
            assert.strictEqual(JSON.parse(readFileSync(join(dir, "state.json"), "utf-8"))
                .schemaVersion, 2);
        }
        finally {
            cleanupTempDir(dir);
    ...
    test("T-SI-04 | schemaVersion 1 → StateVersionMismatchError", () => {
        const dir = makeTempDir();
        try {
            writeFileSync(join(dir, "state.json"), loadFixture("states/version-mismatch.json"));
...
describe("state v2 to v4 migration", () => {
    test("blocks a v2 state whose manifestPath is outside RUN_DIR", () => {
        const dir = makeTempDir();
        try {
            const legacy = JSON.parse(loadFixture("states/mid-run-agent-pending.json"));
    ...
    test("migrates a v2 state without any pending record (no-op)", () => {
        const dir = makeTempDir();
        try {
            writeFileSync(join(dir, "state.json"), loadFixture("states/mid-run-no-pending.json"));
...
describe("v3 to v4 migration", () => {
    ...
    test("blocked: MANIFEST_MISSING when file does not exist inside run dir", () => {
        ...
        try {
            ...
            try {
                ...
                assert.ok(err instanceof StateMigrationBlockedError);
                assert.strictEqual(err.reason, "MANIFEST_MISSING");
            }
        }
        finally {
            cleanupTempDir(dir);
    ...
    test("blocked: MANIFEST_SYMLINK when manifest is a symbolic link", () => {
        ...
        try {
            ...
            try {
                ...
                assert.ok(err instanceof StateMigrationBlockedError);
                assert.strictEqual(err.reason, "MANIFEST_SYMLINK");
            }
        }
        finally {
            cleanupTempDir(dir);
    ...
    test("blocked: MANIFEST_NOT_REGULAR when manifest is a directory", () => {
        ...
        try {
            ...
            try {
                ...
                assert.ok(err instanceof StateMigrationBlockedError);
                assert.strictEqual(err.reason, "MANIFEST_NOT_REGULAR");
            }
        }
        finally {
            cleanupTempDir(dir);
    ...
    test("blocked: MANIFEST_DIGEST_MISMATCH for external request", () => {
        ...
        try {
            ...
            try {
                ...
                assert.ok(err instanceof StateMigrationBlockedError);
                assert.strictEqual(err.reason, "MANIFEST_DIGEST_MISMATCH");
            }
        }
        finally {
            cleanupTempDir(dir);
    ...
    test("successful: v3 delegation manifestPath converted to manifestArtifact", () => {
        ...
        try {
            ...
            assert.strictEqual(artifact.sizeBytes, Buffer.byteLength(manifestContent));
...
describe("writeStateAtomic (T-SI-08..12)", () => {
    ...
    test("T-SI-10 | schema fail → no write at all", () => {
        ...
        try {
            ...
            assert.throws(() => writeStateAtomic(dir, bad, schema), StateCorruptedError);
    ...
    test("T-SI-11 | tmp file left by interrupted write preserves previous state", () => {
        ...
        try {
            writeStateAtomic(dir, buildState({ a: 1 }));
            writeFileSync(join(dir, "state.json.tmp"), JSON.stringify(buildState({ a: 2 })));
    ...
    test("writes and reads one pending external request", () => {
        ...
        try {
            ...
            assert.deepStrictEqual(readState(dir)?.pendingExternalRequest, state.pendingExternalRequest);
        }
        finally {
            cleanupTempDir(dir);
        }
        ...
        try {
            const state = {
                ...
                pendingExternalRequest: {
                    ...
                    emittedAt: "2026-04-19T12:00:00.000Z",
                    emittedAtEpochMs: 1,
                    acceptedResolutionPath: "/tmp/accepted-external-resolutions/push-repo.json",
                },
                usedLabels: ["push-repo"],
            };
            writeFileSync(join(dir, "state.json"), JSON.stringify(state));
...
//# sourceMappingURL=state-io.test.js.map