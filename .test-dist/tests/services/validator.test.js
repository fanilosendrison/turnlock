...
import { z } from "zod";
import { summarizeZodError, validateResult, } from "../../src/services/validator.js";
...
describe("validateResult success (T-VA-01..02)", () => {
    test("T-VA-01 | valid object", () => {
        ...
        if (r.ok)
            assert.deepStrictEqual(r.data, { foo: "a", bar: 1 });
    });
...
describe("validateResult failures (T-VA-03..07)", () => {
    test("T-VA-03 | wrong type foo", () => {
        ...
        if (!r.ok) {
            assert.strictEqual(r.error.issues.some((i) => i.path.includes("foo")), true);
        }
    });
...
describe("summarizeZodError (T-VA-08..10)", () => {
    ...
    test("T-VA-09 | many fields truncated with ellipsis", () => {
        const bigSchema = z.object(Object.fromEntries(Array.from({ length: 50 }, (_, i) => [`f${i}`, z.string()])));
...
describe("validator properties (P-VA-a..c)", () => {
    ...
    test("P-VA-b | summary ≤ 200 chars (fuzz 50 errors)", () => {
        for (let i = 0; i < 50; i++) {
            const fakeSchema = z.object(Object.fromEntries(Array.from({ length: i + 1 }, (_, j) => [`k${j}`, z.string()])));
