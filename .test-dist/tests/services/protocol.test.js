...
import { parseProtocolBlock, writeProtocolBlock, } from "../../src/services/protocol.js";
...
describe("parseProtocolBlock happy (T-PR-13..19)", () => {
    test("T-PR-13 | DELEGATE full", () => {
        const parsed = parseProtocolBlock(loadFixture("protocol/delegate-prompt.txt"));
    ...
    test("parses REQUEST_EXTERNAL fields", () => {
        const parsed = parseProtocolBlock(loadFixture("protocol/request-external.txt"));
        ...
        assert.strictEqual(parsed?.fields.result, "/tmp/external-results/push-repo.json");
    });
    ...
    test("T-PR-15 | ERROR preflight runId=null", () => {
        const parsed = parseProtocolBlock(loadFixture("protocol/error-preflight.txt"));
    ...
    test("T-PR-17 | ABORTED", () => {
        const parsed = parseProtocolBlock(loadFixture("protocol/aborted-sigint.txt"));
...
describe("parseProtocolBlock rejects (T-PR-20..24)", () => {
    ...
    test("T-PR-21 | no @@END@@ → null", () => {
        assert.strictEqual(parseProtocolBlock(loadFixture("protocol/malformed-missing-end.txt")), null);
    });
    test("T-PR-22 | missing @@TURNLOCK@@ → null", () => {
        assert.strictEqual(parseProtocolBlock("version: 3\nrun_id: X\n@@END@@"), null);
    });
...
describe("parseProtocolBlock multiplicity (T-PR-25..26)", () => {
    test("T-PR-25 | two blocks → returns first", () => {
        const parsed = parseProtocolBlock(loadFixture("protocol/malformed-double-block.txt"));
        assert.notStrictEqual(parsed, null);
...
describe("protocol properties (P-PR-a..d)", () => {
    ...
    test("P-PR-b | pure (same input → same output)", () => {
        ...
        assert.strictEqual(writeProtocolBlock("DONE", fields), writeProtocolBlock("DONE", fields));
    });
