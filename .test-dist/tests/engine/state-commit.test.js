...
import { commitStateWithProjection, refreshOwnershipFromContext, releaseOwnershipBestEffort, releaseOwnershipFromContext, } from "../../src/engine/state-commit.js";
...
describe("commitStateWithProjection — strict orThrow", () => {
    ...
    test("expired handle throws AuthorityLostError with reason EXPIRED_HANDLE", () => {
        ...
        try {
            ...
            unsafeEnsureInitialStateRow(ctx.runDb.connection, handle.incarnationId, STATE_SCHEMA_VERSION, JSON.stringify({ schemaVersion: STATE_SCHEMA_VERSION, runId: RUN_ID }), 0, "1970-01-01T00:00:00.000Z");
            const stateRevisionBefore = "0";
            const commitCtx = {
                runDb: ctx.runDb,
                handle,
    ...
    test("revision conflict throws StateRevisionConflictError", () => {
        ...
        try {
            ...
            if (acquired.kind !== "ACQUIRED")
                return;
            const handle = acquired.handle;
            unsafeEnsureInitialStateRow(ctx.runDb.connection, handle.incarnationId, STATE_SCHEMA_VERSION, JSON.stringify({ schemaVersion: STATE_SCHEMA_VERSION, runId: RUN_ID }), epoch, iso);
    ...
    test("DB_FAILURE during best-effort release does not throw", () => {
        ...
        try {
            ...
            assert.doesNotThrow(() => releaseOwnershipBestEffort({
                runDb: ctx.runDb,
                handle: acquired.handle,
                runId: RUN_ID,
                logger,
    ...
    test("successful refresh returns LockHandle and updates ctx.handle", () => {
        ...
        try {
            ...
            assert.strictEqual(newHandle.incarnationId, acquired.handle.incarnationId);
            assert.strictEqual(newHandle.fenceToken, acquired.handle.fenceToken);
            assert.ok(newHandle.leaseUntilEpochMs > acquired.handle.leaseUntilEpochMs);
            // ctx.handle must have been updated.
            assert.strictEqual(refreshCtx.handle.leaseUntilEpochMs, newHandle.leaseUntilEpochMs);
        }
        finally {
            ctx.cleanup();
        }
...
//# sourceMappingURL=state-commit.test.js.map