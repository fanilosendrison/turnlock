...
import {
	...
	type WorkerSignal,
} from "../helpers/orchestrator-crash-harness.js";
...
function assertInitialDispatchClaimCommittedOnResume(
	...
	assert.notStrictEqual(
		after.ownership.ownerToken,
		before.ownership.ownerToken,
	);
	assert.strictEqual(
		BigInt(after.ownership.fenceToken),
...
async function assertPublicResumeRepairsFromSqlite(params: {
	...
	try {
		...
		assert.strictEqual(
			projected.orchestratorName,
			CRASH_TEST_ORCHESTRATOR_NAME,
		);
		assert.strictEqual(
			projected.runIncarnationId,
			params.before.incarnation.id,
		);
		assert.strictEqual(projected.currentPhase, "start");
		assert.strictEqual(projected.stateRevision, "1");
		assert.strictEqual(
			projected.committedFenceToken,
			after.state.committedByFenceToken,
		);
		...
		assert.ok(
			!(PENDING_INITIAL_DISPATCH_VERSION_STATE_FIELD in Object(projected)),
		);
...
describe("orchestrator projection crash recovery", () => {
	test("real initial path dies after bootstrap returns and real --resume repairs and dispatches SQLite state", async () => {
		...
		try {
			...
			assert.strictEqual(
				before.incarnation.orchestratorName,
				CRASH_TEST_ORCHESTRATOR_NAME,
			);
	...
	test("SIGKILL before the initial dispatch claim permits one public resume", async () => {
		...
		try {
			...
			>;
			assert.strictEqual(
				beforeClaim[PENDING_INITIAL_DISPATCH_STATE_FIELD],
				true,
			);
			assert.strictEqual(
				beforeClaim[PENDING_INITIAL_DISPATCH_VERSION_STATE_FIELD],
				1,
			);
			...
			try {
				...
				assert.ok(
					!(PENDING_INITIAL_DISPATCH_VERSION_STATE_FIELD in Object(claimed)),
				);
			} finally {
				output = await killAndCollect(resumed.worker);
			}
	...
	test("SIGKILL after the initial dispatch claim refuses resume before phase entry", async () => {
		...
		try {
			...
			>;
			assert.ok(
				!(PENDING_INITIAL_DISPATCH_STATE_FIELD in Object(claimedState)),
			);
			assert.ok(
				!(PENDING_INITIAL_DISPATCH_VERSION_STATE_FIELD in Object(claimedState)),
			);
			expireOnlyOwnershipLease(dbPath);
			...
			assert.ok(
				resumed.stdout.includes("Initial dispatch was already claimed"),
			);
	...
	test("normal initial execution claims revision 1 before its durable phase result", async () => {
		...
		try {
			...
			assert.ok(
				!(PENDING_INITIAL_DISPATCH_STATE_FIELD in Object(authoritativeState)),
			);
			assert.ok(
				!(
	...
	test("refuses a second public resume after SIGKILL interrupts the first freely executing phase", async () => {
		...
		try {
			...
			assert.ok(
				secondResume.stdout.includes("Initial dispatch was already claimed"),
			);
			assert.deepStrictEqual(readSentinelEntries(sentinelFile), ["start"]);
		} finally {
			cleanupTempDir(dir);
	...
	test("process-only SIGKILL after rename preserves a visible projection without claiming power-loss durability", async () => {
		...
		try {
			...
			assert.strictEqual(
				renamedProjection.runIncarnationId,
				before.incarnation.id,
			);
