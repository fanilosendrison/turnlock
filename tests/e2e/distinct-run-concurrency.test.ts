...
describe("process E2E distinct-run concurrency", () => {
	test("distinct run IDs execute concurrently without cross-run interference", {
		timeout: 30_000,
	}, async () => {
...
`),
		...
		const runs = RUN_IDS.map((runId, index) => ({
			...
			process: workspace.spawnEntrypoint(entrypoint, ["--run-id", runId], {
		...
		try {
			...
			if (!workersFinished) {
				...
				await Promise.all(
					runs.map((run) => run.process.wait(1_000).catch(() => undefined)),
				);
			}
			workspace.cleanup();
		}
