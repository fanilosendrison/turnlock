...
export function validatePendingDelegationV3(value: unknown): void {
	...
	if (
		...
		!isNonEmptyStateString(pending.manifestPath) ||
	...
	if (
		...
		(!Array.isArray(pending.jobIds) ||
			!pending.jobIds.every((jobId) => isNonEmptyStateString(jobId)))
	) {
...
export function validatePendingExternalRequestV3(
	value: unknown,
	runId: string,
